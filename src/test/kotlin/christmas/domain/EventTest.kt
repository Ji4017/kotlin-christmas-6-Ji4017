...
class EventTest {
    ...
    fun `평일 할인 적용 테스트`() {
        val dessertCount = 3
        val orderInformation = Order(6, mapOf("초코케이크" to dessertCount, "양송이수프" to 2))
        val benefit = Event(orderInformation)
        val discounts = benefit.getDiscounts()
    ...
    fun `주말 할인 적용 테스트`() {
        val mainCount = 2
        val saturDay = 9
        val orderInformation = Order(saturDay, mapOf("티본스테이크" to mainCount, "양송이수프" to 2))
        val benefit = Event(orderInformation)
        val discounts = benefit.getDiscounts()
    ...
    fun `특별 할인 적용 테스트`() {
        val starDay = 3
        val orderInformation = Order(starDay, mapOf("양송이수프" to 2, "타파스" to 1))
        val benefit = Event(orderInformation)
        val discounts = benefit.getDiscounts()
    ...
    @Test
    fun `증정 이벤트 적용 테스트`() {
        val orderInformation = Order(6, mapOf("해산물파스타" to 2, "레드와인" to 1, "초코케이크" to 1))
        val benefit = Event(orderInformation)
        val discounts = benefit.getDiscounts()
    ...
    fun `증정품 경곗값 테스트`() {
        ...
        val orderWithGift = Order(6, mapOf("양송이수프" to 10, "레드와인" to 1))
        val given = Event(orderWithGift)

        // 주문의 총 가격이 선물 증정 가격 이하
        val orderWithoutGift = Order(6, mapOf("양송이수프" to 2, "타파스" to 1))
